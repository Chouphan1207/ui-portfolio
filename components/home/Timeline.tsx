...
// 1. Create a dedicated container for clean, independent fade-in triggers
function TimelineCard({ item }: { item: TimelineEntry }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 40 }} // Starts invisible and pushed down slightly
      whileInView={{ opacity: 1, y: 0 }} // Smoothly transitions in when scrolled into view
      viewport={{ once: true, margin: "-100px" }} // Triggers once, slightly before it hits the center viewport
    ...
    >
      ...
      <div className="sticky flex items-center md:items-start z-40 top-24 self-start w-full">
        {/* Timeline Indicator Dot (Hidden on mobile) */}
        <div className="hidden md:flex h-8 w-8 absolute left-3 rounded-full items-center justify-center bg-primary border border-[--border] shadow-xs dark:border-white/10">
...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div className="w-full bg-background font-sans px-4 md:px-10" ref={containerRef}>
      {/* Header Info Block with its own subtle entrance */}
      <motion.div
